...
export const mailOTPController = async (request, response) => {
    ...
    try {
        ...
        const user = await UserModel.findOne({ email })
        if (!user) return response.status(500).json({ error: 'Email not found' })
...
export const updateProfileController = async (request, response) => {
    ...
    try {
        ...
        if (profilePicture) {
            ...
            // Checking whether the uploaded images is in base64 fomat rather than a URL or a path
            const imageRegex = /^data:image\/(png|jpe?g|gif|webp|bmp|svg\+xml);base64,[A-Za-z0-9+/]+=*$/
            if (typeof profilePicture !== 'string' || !imageRegex.test(profilePicture))
                return response.status(400).json({ error: 'Invalid profile picture' })

            // Checking whether the image exceeds more than 3MB
            const img = profilePicture.split(',')[1]
            const byteSize = Buffer.byteLength(img, 'base64')
            const MAX_SIZE = 3 * 1024 * 1024 // 3MB
            if (byteSize > MAX_SIZE) return response.status(400).json({ error: 'Image size should not exceed 3MB' })
